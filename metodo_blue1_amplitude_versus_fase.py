...
2) Função para a definição do vetor da derivada temporal do pulso de refererência.
Entrada: número de janelamento.
...
def metodo_BLUE1(Matriz_pulsos_sinais_teste, vetor_amplitude_referencia_teste, vetor_fase_referencia_teste, Matriz_covariancia, n_janelamento):
    ...
    for indice_linha in range(len(Matriz_pulsos_sinais_teste)):
        ...
        # Cálculo do vetor do parâmetro estimado pelo método BLUE 1.
        vetor_parametro_estimado = np.dot(vetor_pesos_blue1, vetor_pulsos_sinais_teste)
        
        # Cálculo do termo da amplitude versus a fase estimada.
        amplitude_versus_fase_estimada = vetor_parametro_estimado[1]
        
        # Cálculo do erro de estimação do termo da amplitude versus a fase.
        erro_amplitude_versus_fase = valor_amplitude_versus_fase_referencia_teste-amplitude_versus_fase_estimada
    
        # O elemento erro_amplitude é adicionado na lista correspondente.    
        lista_erro_amplitude_versus_fase.append(erro_amplitude_versus_fase)

    # A função retorna a lista lista_erro_amplitude.
    return lista_erro_amplitude_versus_fase
